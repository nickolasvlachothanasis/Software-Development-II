...
import outputManagePackage.OutputManagePackageTxt;
import outputManagePackage.OutputManagePackageXml;
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (taxpayerIndex!=-1){
					...
					if(saveFileFolderChooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
					    ...
					    OutputManagePackageTxt outputSystemTxt = new OutputManagePackageTxt(saveFileFolderChooser.getSelectedFile(), taxpayerIndex);
					    outputSystemTxt.saveTaxpayerInfoToLogFile(savePath, taxpayerIndex);
					}
				}
				else{
					JOptionPane.showMessageDialog(null, "Δεν έχεις επιλέξει κάποιον φορολογούμενο απο την λίστα.", "Σφάλμα", JOptionPane.WARNING_MESSAGE);
		...
		saveSelectedTaxpayerInfoToXmlButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (taxpayerIndex!=-1){
					...
					if(saveFileFolderChooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
					    String savePath = saveFileFolderChooser.getSelectedFile().toString();
					    OutputManagePackageXml outputSystemXml = new OutputManagePackageXml(saveFileFolderChooser.getSelectedFile(), taxpayerIndex);					    
					    outputSystemXml.saveTaxpayerInfoToLogFile(savePath, taxpayerIndex);
					}
				}
				else{
					JOptionPane.showMessageDialog(null, "Δεν έχεις επιλέξει κάποιον φορολογούμενο απο την λίστα.", "Σφάλμα", JOptionPane.WARNING_MESSAGE);
